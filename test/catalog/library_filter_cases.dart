...
void main() {
  ...
  testWidgets(
    'library refresh failure keeps cards and retries without resetting scroll',
    (tester) async {
      await pumpLoggedIn(tester);
      await openLibrary(tester, 'view-movies');
      final before = posterNames(tester);
  ...
  testWidgets(
    'library page failure retains loaded page and retries the same offset',
    (tester) async {
      for (var i = 0; i < 80; i++) {
        server.items.add(
          FakeEmbyItem(
        ...
        server.requests
            .where(
              (request) =>
                  ...
                  request.contains('StartIndex=60'),
            )
            .length,
        1,
      );
  ...
  testWidgets(
    'filtered library preserves query and scroll after returning from detail',
    (tester) async {
      for (var i = 0; i < 80; i++) {
        server.items.add(
          _GenreItem(
      ...
      final poster = tester
          ...
          .firstWhere(
            (card) => find
                .byKey(CatalogKeys.item(card.item.id))
                .hitTestable()
                .evaluate()
                .isNotEmpty,
          );
      await tester.tap(find.byKey(CatalogKeys.item(poster.item.id)));
      await settle(tester);
      ...
      scroll.jumpTo(0);
      await settle(tester);
      expect(find.byKey(gridFilterClearKey), findsOneWidget);
      expect(
        posterNames(tester).every((name) => name.startsWith('科幻电影')),
        isTrue,
      );
    },
    tags: ['integration'],
