...
def capture_audio(serial, output, rate, seconds=6):
    ...
    stream = stub.streamAudio(request, metadata=metadata, timeout=seconds)
...
def app_flow(device, tv):
    ...
    if tv:
        d.key(22)
        source_focus = focused_labels(d.wait(lambda s: bool(focused_labels(s)), 'source card focus'))
        d.key(23)
    ...
    d.wait(lambda s: s['player'] and not s['player']['loading'] and s['player']['playing'], 'real Media3 playing', 40)
    try:
    ...
    if not tv:
        ...
        d.wait(lambda s: s['size'][1] > s['size'][0], 'restore portrait')
    if tv:
        d.key(85)
    else:
        d.tap(key='mobile-player-toggle')
    ...
    while time.monotonic() < deadline:
        ...
        for stage in ['embedded-subtitle', 'external-srt', 'external-vtt', 'surface-reconnected-audio-capture']:
            if '"stage":"' + stage + '"' in log and stage not in captured:
                captured.add(stage)
                first = d.screenshot('native-' + stage)
                if stage == 'surface-reconnected-audio-capture':
                    time.sleep(1)
                    result['pixels'] = pixel_check(first, d.screenshot('native-moving-b'))
        ...
        if args.apk:
            ...
            for _ in range(50):
                ...
                if (output / 'fixture/server.json').exists():
                    break
                time.sleep(.1)
            app = args.app_apk or output / 'app-probe.apk'
            if not args.app_apk:
            ...
            native_server = subprocess.Popen([sys.executable, str(ROOT / 'packages/rillight_android_player/tool/smoke_server.py'),
