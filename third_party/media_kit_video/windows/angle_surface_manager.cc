...
void ANGLESurfaceManager::Read() {
  ...
  if (d3d_11_device_context_ != nullptr) {
    ...
    if (copy_fence_query_ != nullptr) {
      d3d_11_device_context_->End(copy_fence_query_.Get());
    }
...
void ANGLESurfaceManager::WaitForCopy() {
  if (copy_fence_query_ == nullptr || d3d_11_device_context_ == nullptr) {
    return;
  }
  // |Flush| only submits the copy; the shared handle is read by Flutter's own
  // D3D11 device with no cross-device synchronization, so spin until the GPU
  // reports the copy as complete. Bounded so a lost device cannot hang the
  // raster thread: any error (e.g. DXGI_ERROR_DEVICE_REMOVED) or timeout falls
  // back to the previous unsynchronized behavior for this frame.
  constexpr ULONGLONG kMaxWaitMs = 100;
  const auto start = ::GetTickCount64();
  for (;;) {
    const auto hr = d3d_11_device_context_->GetData(copy_fence_query_.Get(),
                                                    nullptr, 0, 0);
    if (hr != S_FALSE) {
      return;
    }
    if (::GetTickCount64() - start > kMaxWaitMs) {
      return;
    }
    ::SwitchToThread();
  }
...
bool ANGLESurfaceManager::CreateD3DTexture() {
  ...
  if (copy_fence_query_ == nullptr) {
    // Best effort: if the query cannot be created, |Read| simply skips the
    // wait and behaves like upstream media_kit.
    D3D11_QUERY_DESC query_desc{};
    query_desc.Query = D3D11_QUERY_EVENT;
    query_desc.MiscFlags = 0;
    if (FAILED(d3d_11_device_->CreateQuery(&query_desc, &copy_fence_query_))) {
      copy_fence_query_.Reset();
      std::cout << "media_kit: ANGLESurfaceManager: Failure: "
                   "ID3D11Device::CreateQuery (copy fence disabled)"
                << std::endl;
    }
  }
