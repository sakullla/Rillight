...
class ANGLESurfaceManager {
 ...
 private:
  // Rillight patch: block until the GPU has finished the |Read| copy so that
  // Flutter (which samples |handle_| on a different D3D11 device without any
  // fence) never sees a half-written or empty texture. Without this, frames
  // occasionally tear or flash black on loaded GPUs / high refresh displays.
  void WaitForCopy();
  ...
  // Rillight patch: event query signalled once the |Read| copy has executed.
  Microsoft::WRL::ComPtr<ID3D11Query> copy_fence_query_;
  // ANGLE
